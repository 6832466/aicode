"""Excel 导出 —— 纯函数，从 scrape_douyin_detail.py 的 save_to_excel 提取。"""
...
def export_to_excel(all_data: dict, output_path: str) -> int:
    """保存到 Excel，每个分类一个 Sheet + 汇总 Sheet，返回总记录数。"""
    wb = Workbook()
    wb.remove(wb.active)
    ...
    for cat_name, cards in all_data.items():
        ...
        sheet_name = base[:31] if len(base) <= 31 else base[:27] + "_" + hex(abs(hash(cat_name)) % 0xFFF)[2:]
